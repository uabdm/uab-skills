...
center. This flows silently into README.md's "Who to contact" section — see
LIFECYCLE & GOVERNANCE in `decision-matrix.md`. Never shown back to the
project leader as a "technical" requirement.
...
## What happens next
Once they say "looks good", the app will be built and tested to confirm it
actually works, then packaged into a single zip file you can download. It
will run on your computer (or a developer's) with one or two commands
listed in the included instructions. Getting it set up somewhere permanent

...
- UAB brand theme + WCAG 2.1 AA accessibility, generated by calling the
  `uab-branding` skill (see `scaffold-web-nextjs.md`'s LAYOUT section for
  exactly where) — sourced from and verified against MyUABPortal, UAB's
  real production portal, not re-fetched from the public brand guide site
  per generation.
